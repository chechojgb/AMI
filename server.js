...
// Conexión al AMI
// Nota: Puedes cambiar la IP, usuario, password, etc. según corresponda.
...
/**
 * Función auxiliar para ejecutar un comando AMI utilizando la acción "Command".
 * Se utiliza en el endpoint /queue/remove2 para enviar comandos como "sip unregister" y "sip reload"/"pjsip reload".
 * @param {string} command - El comando a ejecutar.
 * @returns {Promise} Promesa que se resuelve con la respuesta del AMI.
 */
async function executeAMICommand(command) {
...
app.post('/queue/remove2', async (req, res) => {
  ...
  try {
    ...
    // 2. Intento de desregistro.
    // Para chan_sip se espera que solo se envíe el número (ej. "7005"), por ello se extrae del valor "SIP/7005".
    let unregisterResult = null;
    if (channel.startsWith('SIP/')) {
      const extensionNumber = channel.split('/')[1]; // Extrae "7005" de "SIP/7005"
      unregisterResult = await executeAMICommand(`sip unregister ${extensionNumber}`);
    } else {
      unregisterResult = { message: 'No se aplica desregistro forzado en pjsip' };
    }

    // 3. Recargar la configuración del canal.
    // Se utiliza "sip reload" para chan_sip y "pjsip reload" para pjsip.
    let reloadCommand = channel.startsWith('SIP/') ? 'sip reload' : 'pjsip reload';
    const reloadResult = await executeAMICommand(reloadCommand);

    return res.json({
      message: `Para ${channel} se ejecutaron: eliminación de colas (${queues.join(', ')}), intento de desregistro y recarga del canal.`,
      results: {
...
/**
 * Endpoint para consultar el estado del canal de una extensión.
 */
...
/**
 * Endpoint para consultar el estado de una extensión y las colas en las que se encuentra.
 */
...
/**
 * Endpoint para consultar el estado de una extensión y obtener detalles de las colas en las que se encuentra.
 */
...
/**
 * Endpoint para colgar una llamada pegada.
 * Método: GET
 * Ruta: /channel/hangup2/:extension
 */
